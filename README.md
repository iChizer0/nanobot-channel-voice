...
- **Defaults** are the schema's, beneath `$NANOBOT_VOICE_DEFAULTS` in the gateway environment when set (a partial section, as a JSON file path or the JSON itself): how a board image ships its baseline, which a fresh config runs as is and Reset returns to.
