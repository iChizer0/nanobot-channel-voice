...
- **Apply** fetches the models the resolved setup runs, then starts or restarts the channel. A `weights` key an earlier pick left behind is not fetched, though `nanobot-voice sync` fetches every named key. Apply also removes the models it installed (`managed_by: webui` in their manifests) that the setup no longer runs; anything fetched by hand stays until `nanobot-voice prune` or `sync --prune`. While the section's index has not loaded, an Apply that would download refuses, saying why; one with nothing to download goes ahead.
