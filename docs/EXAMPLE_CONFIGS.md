...
An index is read over https or from a file on this machine (plain http only from the machine itself): it pins every file's sha256, so it has to arrive authenticated, while the files it lists may come over anything. A file `url` may be relative to the index, so a copy of an index with its files beside it, on a mirror or a disk, serves them from there; one that names its files absolutely downloads them from there wherever it is read. `nanobot-voice list`, `fetch` and `sync` read the index from the config the gateway reads (`--config` for another; with no config file, a board's defaults), and `--index` overrides it for one run, leaving the panel's cached index alone.

The WebUI does the same from the Voice panel: each engine's **Model** pills come from the index the section names (**Model index** under General, under Advanced while it is the built-in one: any other shows without it, being where every download comes from, a pasted section's included, and its help names the host an index lists its files on when that is not the index's own; cached in the store as `.index.json`, which is what the panel opens on, at once and offline too, and reloaded in the background whenever the panel opens or the section names another index, the pills following once the reload lands; an index that does not load is named with its error under **Models**, retried there, and an Apply with a model to download refuses, saying why, until the one the section names has loaded, one with nothing to download applying regardless) filtered to the builds this setup runs: `onnx` always, plus `rknn.<device>` when `channels.voice.device` names the SoC (`"rv1126b"`, `"rk3588"`; the **Device** row takes the name as typed, its note lists the chips the index has builds for, and an engine the filter leaves with no model at all says which chips the index has it for), which is also the RKNN target of every engine block unless a block sets its own `target`. Pills carry each model's languages from the index's `langs`. Each pill is one model; a model built for the CPU and for the chip shows a **Build** row under the pill, the chip build picked by default, CPU there for when the NPU is busy with speech-to-text and text-to-speech. Picking an engine also picks its first listed model, so a Model row never opens on nothing; picking a cloud provider's **On speech** or **After wake word** picks Silero with its first model while the detector is not neural, since the gate refuses Energy (every false onset is a billed upload) and the detector's row is advanced; **After wake word** also turns the wake **Mode** on, the uplink refusing a wake gate without one, so the **Wake word** section follows the pick, its **Model** row waiting for the head (which phrase is yours to choose), and **Off** there keeps the gate **On speech**; leaving **After wake word** for another pick takes the mode back off again while nothing has filled its phrase, the pick having turned it on. The wake word runs on the cloud path only as that gate (no transcript tier, no spoken acknowledgement and no cue: a cloud summon has no audible receipt), which is why the section sits under **Send audio** rather than beside it. For the wake word, **Off** is the Mode row alone; a mode on shows the **Model** row first and it switches the tier: **Transcript** (the text tier alone, local backend only), the index's openWakeWord heads read by phrase, and **Custom**, a head you trained yourself (`wake.openwakeword.modelPath`, its shared feature models fetched by Apply as `wake/openwakeword/backbone/<platform>`, an entry the plugin derives from any head when the index has none of its own). The pick sets `wake.engine`; a head also fills **Phrases** with the phrase it hears (the package's `meta.json` once installed, the index's naming before) unless the list has it, the **Phrases** row saying so if you edit it away. The rows a listener tunes follow their switches: the served endpoint's host, port and key under **Serve transcription**, an **Interruptions** section whose switch is **Echo cancellation** (`aec`: whether the microphone stays open while a reply plays; the open-mic modes add barge-in Duck or Pause where the voice streams, the duck level while ducking, the words that cut in, the heard marker), a **Waiting** section (thinking filler with its timing and phrases, and the **Agent timeout**, which arms the watch: the stall notice and both phrases follow it, an empty timeout speaking nothing), the wake **Attention**, **Window**, **Aliases** and spoken **Acknowledge**, a **Cues** section for the earcons, a neural detector's **Model** and **Threshold** and Smart Turn's **Model** under **Listening**, and for a cloud provider its **Persona**, **Park after**, xAI's **Reasoning**, and under an Open mic **Barge-in** the **Echo cancellation** it needs (WebRTC or Hardware, the cloud path having no transcript to filter a reply out of; a Resolved setup row says so while neither is picked, both rows being advanced). The on-device engines a cloud backend loads get a Resolved setup row of their own, named after what asks for them (**On speech**, **After wake word**, or **Serve transcription** under Continuous), when one would not load: the detector, the head or the served engine with no model or not fetched yet, the channel not starting without it, Smart Turn alone degrading to silence-only turns. On the local path the same holds for the served engine: with **Serve transcription** on, the pipeline row says the channel does not start without it instead of naming a stand-in. A number's unit sits at the end of its input (ms, s, dB, words), never in the label. Every row shows its resolved value. An emptied input is null where the schema allows it (the clocks, a provider model, a clip path, the phrase lists, **Device**) and an empty list for a list, so the help's "Empty" is what you see; elsewhere it withdraws the edit and the row shows the resolved value again. The selected model's license line sits under the pills, above the row's help (the index's `license`, then its `accept` notice); the same notice sits with a tick beside the model under **Models** once Apply would fetch it, Apply waits for the tick and says so, and the Resolved setup's pipeline row (or a cloud gate's row) names the model whose notice holds it. The rows a first setup does not need (the hardware facts, **Device**, the built-in **Model index**, **Microphone**, **Speaker** and **Audio backend**, set once per machine and by a board image's defaults, so **Audio** shows under Advanced only while the Resolved setup names the devices that open; the audio-path details, **Echo cancellation** and **Barge-in**, local and cloud, so **Interruptions** too shows under Advanced only, and an OpenAI-style voice's **Audio format**, PCM streaming so a reply can pause for you; the listening internals, **Voice activity detector**, **End of speech** and **End of turn** with their models, so **Listening** shows under Advanced only as well, the identity line and the Local pipeline row naming what listens and speaks, and any engine that would not load or lacks a model; then the thresholds and timings, an on-device voice's **Speed** among them (a float, 1 being the voice's own pace), **Serve transcription** and its address, **Heard marker**, the wake **Attention** and **Aliases**, the cue clips, the phrase lists, **Allowed senders**, **Log transcripts**, a cloud provider's **Park after**, **Tool mode** and **Reasoning**) sit behind the panel's **Advanced** toggle, in place in their sections, a run of them shaded as one block so what the toggle added stands out from the rows that were already there (the Advanced area says so, under its buttons). So does every section this setup does not use, with a note under its legend naming the switch that would: **Waiting** and **Interruptions** while replies are not spoken, and for a cloud provider **Listening** and **Wake word** until **Send audio** is On speech or After wake word, **Speech-to-text** (the provider transcribes for itself; **Serve transcription** runs an on-device engine for other clients) and the **Device** and **Model index** rows until an on-device detector or served engine runs. The normal view is what the setup uses, Advanced is everything. **Reset to defaults** there writes the pending patch `{"$reset": true}`: the section's own keys go, `enabled` alone kept, the rest of the patch (later edits, or the keys pasted after the directive) stacks on top, and the defaults show through every key it does not set. The defaults are the schema's, beneath the section `$NANOBOT_VOICE_DEFAULTS` names in the gateway environment (a JSON file path or the JSON itself, wrapped like a paste or bare): a layer under `channels.voice`, read wherever the section is resolved, so that a key the section leaves unset, in either spelling and block by block, comes from the operator's section and then from the schema, and what the section sets, a null included, stays as written. That is how a board image ships its baseline (device, audio devices, models), as a partial section: a fresh config runs it as is (`nanobot onboard` writes `enabled` alone), the pane shows it resolved, `nanobot-voice config` prints it, `nanobot-voice sync` fetches its models, and Reset returns to it without copying it in, so a baseline changed later still reaches every key the user left alone. An unreadable defaults file refuses the section outright, the schema row naming the variable. A hand-written `importJson` may carry the directive too, so a pasted whole section becomes exact instead of a merge. **Apply** fetches what the resolved setup runs, the selected engines' models (a `weights` key an engine pick left behind is not fetched; `nanobot-voice sync` still fetches every named key), before starting the channel. Apply also removes models it installed earlier that the setup no longer runs (their manifests carry `managed_by: webui`); anything fetched by hand stays until `nanobot-voice prune` or `sync --prune`.
