...
**Spoken acknowledgment** (`wake.ack.enabled`): a bare summon otherwise opens the attention window in silence - the user cannot hear the difference between "listening" and "didn't hear you". With the ack on, every bare-wake settle answers ("hey nanobot" -> *"I'm here."*, "小娜" -> "在呢。"), and in half-duplex the ack *is* the beat: wait for it, then speak. Phrases rotate; `ack.phrases` overrides the language-keyed built-ins (keep them under ~0.6 s spoken - the half-duplex mic is gated while one plays, and an ack must never contain a wake phrase). The ack routes its language by the *name that was called* when the wake phrases span scripts, falling back to the TTS engine's language; with the acoustic engine on open-mic setups, a bare summon is answered early (~350 ms after the phrase) instead of waiting out the endpoint. The agent never sees the ack - like prologue fillers it is channel-canned audio: it ducks under user speech, any real turn flushes it, and it never produces an "interrupted" note.
