...
- "Still working on it" plays during long tool runs: that is the deadman's first-budget notice (`agentTimeoutS`, default 300 s per budget - the run is killed only after a second silent budget). Healthy long tools push the clock via progress traffic, so leave core's `channels.sendProgress` on; raise `agentTimeoutS` only for models that genuinely think longer than 5 minutes in silence.
