...
Every engine runs in-process (`[ondevice]` extra), no servers or network. The file extension picks the runtime per engine: `.onnx` -> the configured execution provider, `.rknn` -> the RKNN runtime. Weights are never bundled: name every file yourself, or let `nanobot-voice` provision them from an index so an engine block names one store key (`weights`) instead of a path per file. Default index: [nanobot-channel-voice-models](https://huggingface.co/iChizer0/nanobot-channel-voice-models) (currently RKNN builds). `channels.voice.index` names others in its place, merged in order, a later one winning for the same model, and an empty list is no index at all (only the models already in the store); a board image's `$NANOBOT_VOICE_DEFAULTS` can carry it like any other key:

```json
{"channels": {"voice": {"index": ["https://example.com/voice-weights.json"]}}}
```

```sh
nanobot-voice sync        # fetch every weights key the config names, from the index it names, sha256-verified
```

An index is read over https or from a file on this machine (plain http only from the machine itself): it pins every file's sha256, so it has to arrive authenticated, while the files it lists may come over anything. A file `url` may be relative to the index, so a copy of an index with its files beside it, on a mirror or a disk, serves them from there; one that names its files absolutely downloads them from there wherever it is read. `nanobot-voice list`, `fetch` and `sync` read the index from the config the gateway reads (`--config` for another; with no config file, a board's defaults), and `--index` overrides it for one run, leaving the panel's cached index alone. A key the index has renamed stays for a while as a deprecated alias: a config naming it still installs, `nanobot-voice` names the new key, and the panel offers only that.

The WebUI's Voice panel does the same: each engine's **Model** pills come from the index the section names, and **Apply** fetches what the setup runs before starting the channel (see [The Voice panel](#the-voice-panel)).
...
The same config with `.onnx` paths validates a split off-board before RKNN conversion (do not set `acousticModelPath`). Bucket geometry and mel statistics resolve from config (`encoderLen`/`melLen`/`melScale`/`melBias`), else an exporter `meta.json` beside the models (or `metaPath`), else the en_US-ljspeech values (200/800, `2.0661438`/`-5.5238085`). The sidecar may also declare the frontend (`"frontend": "zh-en-lexicon" | "lexicon" | "espeak"`) plus `sample_rate`, `pad_id`, and `use_eos_bos` when the token-table conventions ("_" pad, ^/$ framing, 22.05 kHz) don't hold: `"zh-en-lexicon"` is how a split of the bilingual zh-en model (16 kHz) becomes usable - undeclared zh-en artifacts are refused, as is a lexicon/espeak declaration beside zh-en tokens (wrong sidecar). Output rate follows the sidecar (22,050 Hz default); a piece overflowing a bucket is halved and retried - long content costs a pause, not an error.
...
The same shape with `.rknn` artifacts (`[ondevice,rknn]` extras): Whisper STT, MMS TTS, and FireRed VAD can be provisioned from an index with `nanobot-voice sync` (the CLI may prompt to accept model licenses). `device` names the SoC once: the builds the WebUI offers, and every engine's RKNN target.
...
`wake.mode` gates the pipeline behind a wake phrase (local backend only). `"gate"` requires the phrase to *start* a conversation from cold; follow-ups and barge-in stay natural for `windowS` seconds after each turn (default 15 s). `"strict"` additionally requires it to *interrupt* a live reply - non-wake speech never stops playback: the posture for public/multi-speaker spaces. Under echo cancellation with `bargeIn.mode: "pause"`, speech over the reply pauses it while the phrase can still come (an utterance's first 2.5 s, or a shorter one until it is judged), so the phrase is heard clear of the canceller's double-talk suppression; without the phrase the reply resumes. An utterance that is *only* the phrase publishes nothing: over an audible reply it stops it and listens; while the agent is still *working* it never cancels the query - it answers and the reply still comes. Saying the name when it was not needed is free: redundant and repeated phrases ("小娜小娜") are stripped wherever they land, so when unsure whether the device heard you, just say the name again.
...
**One name, one sentence** (`wake.attention: "sentence"`): the default `"conversation"` re-opens attention for `windowS` after every turn - natural follow-ups, at the cost of an invisible open window. `"sentence"` opens it *only* on wake evidence and the next published turn spends it: the predictable contract for public/noisy rooms. A reply that ends with a question re-opens it so the agent's clarification gets its answer; anything else needs the name again, which the ack makes cheap.
...
- An on-device engine like any other: a `weights` key provisions it (e.g. `wake/openwakeword/hey-jarvis/rknn.rv1126b` via `nanobot-voice fetch`), a missing model degrades loudly to transcript-tier gating. The embedding runs on the NPU whenever there is an NPU build, the `device`'s or, with none named, the one the head comes in: a head in another build takes the chip's pair from `wake/openwakeword/backbone/<platform>`, which `nanobot-voice sync` and Apply fetch beside it, and runs on its own package's pair until then. The mel frontend runs on the CPU (NPU builds carry `melFiltersPath`, an exact numpy port of the mel graph, whose STFT ops don't convert), and the head is always ONNX on the CPU: a `.rknn` head is refused, and the block's `executionProviders` do not apply to it. In the WebUI the wake **Model** pills are the index's heads, and **Custom** is the self-trained case: it clears the key and opens the path rows for the head, the pair coming from the backbone package (an explicit path also wins over the picked model's file, so one set beside a model stays in view). The `meta.json` sidecar's declared phrase/target are checked against the config at startup (advisory warnings).
...
The robust-tools variant, on a core that passes the tool gateway: the realtime model owns the conversation but delegates every reasoning and tool step to nanobot's full agent loop (`realtime.toolMode: "supervisor"`), with the software AEC (`[aec]` extra) keeping the mic open so you can cut it off mid-reply. `delegationTimeoutS` budgets the agent's tool work. A delegation is one ordinary turn in the voice session; while it runs the session waits with the mic open even without AEC (after the filler, the state is THINKING, not SPEAKING), and talking does not cancel it: the model can answer an aside, and the result is spoken once you are done. A stop ends it, and a new request the model makes after hearing you replaces it. With `inputTranscriptionModel` set, a stop phrase is matched on the transcript; otherwise, and always on Gemini, which matches no stop phrases, the model ends the work itself with its `cancel_nanobot` tool. On the official core the same file runs as an open-mic persona-only session - the AEC and barge-in parts still apply.
...
`backend: "gemini"` speaks the Live API's own protocol (`gemini-3.8-live` by default; `gemini-3.8-live-extended-thinking` for background reasoning with `thinkingLevel`, which that model requires and the base model refuses: unset it is `low` there, and on other models it is ignored). The key is `realtime.apiKey` or `GEMINI_API_KEY` / `GOOGLE_API_KEY` - never the OpenAI fallback. Voices are Gemini's prebuilt names (`Kore` default). Tool calls are declared non-blocking (the extended-thinking model requires it and narrates its own waiting; the base model keeps talking while the tool runs), the supervisor's answer interrupts, a direct tool's result waits for the model to go idle, and either waits while an on-device uplink hears you talking. Talking over the turn that issued a call makes the server withdraw the call, and its work stops there (a delegation's nanobot turn is stopped), since no answer can land. `proactiveAudio: true` lets the model decide whether speech was for it - permanently on for the 3.8 Live models, extended thinking included - so an unanswered utterance is normal there, not a fault; it connects through the Live API's `v1alpha` endpoint, the only one that accepts it, unless `realtime.baseUrl` names another. Session resumption and context-window compression are always on (the socket lives ~10 min, an audio-only session 15 min without them); a resumption handle older than the vendor's two hours is dropped, so a device parked overnight starts a fresh conversation instead of failing to reconnect. No playback-aligned truncation exists on this protocol: after a barge-in the model remembers what it sent, not what you heard.
...
On a core that ships the voice panel, **Settings -> Channels -> Voice** is a form the channel shapes for its own state and rebuilds after every edit: the normal view is what the setup uses, **Advanced** is everything. Every row shows its resolved value. Emptying an input writes null where the schema allows it and an empty list for a list; elsewhere it withdraws the edit.

### Models and Apply

- An on-device engine's **Model** pills come from the index the section names, **Model index** under General. The row is advanced while it holds the built-in index; any other shows in the normal view, since every download comes from it, and its help names the host the index lists its files on when that differs.
- The index is cached in the store (`.index.json`), so the panel opens on it at once, offline too, and reloads it in the background when the panel opens or the section names another index. An index that does not load shows its error under **Models**, with a Retry.
- The pills are the builds this setup runs: `onnx` always, plus `rknn.<device>` when `channels.voice.device` names the SoC (`"rv1126b"`, `"rk3588"`), which is also every engine block's RKNN target unless the block sets its own `target`. The **Device** help lists the chips the index has builds for. Each pill is one model with its languages; a model built for both CPU and chip gets a **Build** row, the chip build by default, CPU for when the NPU is busy. A wake head gets none: its feature models follow **Device** whichever build it comes in.
- The selected model's license shows under the pills. One whose license carries a notice (`accept`) is fetched only once you tick it under **Models**; until then Apply says it waits, and the Resolved setup names the model.
- **Apply** fetches the models the resolved setup runs, then starts or restarts the channel. One already in the store that the index has changed since (a file under another sha256, missing, added or dropped, or an archive left packed that this Python can now unpack) is fetched again, only what changed downloading, and **Models** lists it as an update, its notice waiting for a tick like a new model's. An update that cannot land (offline, a blocked hub) holds nothing up: the installed model stays and runs, and the panel says which stayed. A `weights` key an earlier pick left behind is not fetched, though `nanobot-voice sync` fetches every named key and brings each up to date. Apply also removes the models it installed (`managed_by: webui` in their manifests) that the setup no longer runs; anything fetched by hand stays until `nanobot-voice prune` or `sync --prune`. The tag records who installed a model: an update or a re-check, from Apply or the CLI, leaves it as it was. While the section's index has not loaded, installed models are left as they are, and an Apply that would download a new one refuses, saying why; one with nothing to download goes ahead.

### Picks that set other rows

- Picking an engine also picks its first listed model, so a **Model** row never opens empty.
- On a cloud provider, **On speech** and **After wake word** pick Silero while the detector is not neural: the gate refuses Energy, since every false onset is a billed upload.
- **After wake word** also turns the wake **Mode** on, the uplink refusing a wake gate without one. Turning that mode off moves the gate back to **On speech**, and leaving **After wake word** turns the mode back off while no phrase has been filled.

### Wake word

- With a wake mode on, the **Model** row switches the tier: **Transcript** (the text tier alone, local backend only), one of the index's openWakeWord heads, or **Custom**, a head you trained yourself (`wake.openwakeword.modelPath`, whose shared feature models Apply fetches as `wake/openwakeword/backbone/<platform>`).
- The pick sets `wake.engine`, and a head adds the phrase it hears to **Phrases**.
- On a cloud backend the wake word runs only as the uplink gate, with no transcript tier, spoken acknowledgement or cue, which is why the section sits under **Send audio**.

### Rows that follow their switches

A row appears once the switch that makes it matter is on, such as the served endpoint's address under **Serve transcription** or the barge-in details once **Echo cancellation** keeps the mic open. **Agent timeout** arms the whole watch: the stall notice and both phrases follow it, and an empty timeout speaks nothing. On a cloud provider, an open-mic **Barge-in** brings the **Echo cancellation** it needs, WebRTC or Hardware.

### Resolved setup

These rows say what will run. The on-device engines a cloud backend loads get a row of their own, named after what asks for them (**On speech**, **After wake word**, or **Serve transcription** under **Continuous**): one with no model, or not fetched yet, keeps the channel from starting, except Smart Turn, which degrades to silence-only turns. On the local backend a served engine that would not load keeps the channel from starting too, where the other engines fall back.

### Advanced

The rows a first setup does not need open in place, shaded: the hardware facts (**Device**, the built-in **Model index**, **Microphone**, **Speaker**, **Audio backend**), the audio-path and listening details, then thresholds, timings, phrase lists and the access list. **Audio**, **Interruptions** and **Listening** hold nothing else, so they show under Advanced alone, while the identity line and the Resolved setup name what listens, speaks and opens. A value set in a hidden row stays in force.

The sections this setup does not use show there too, each with a note naming the switch that would use it. On a cloud provider that includes **Speech-to-text**: the provider transcribes for itself, and **Serve transcription** runs an on-device engine for other clients.

### Reset and the operator's defaults

**Reset to defaults** writes the pending patch `{"$reset": true}`: the section's own keys go (`enabled` stays), later edits stack on top, and the defaults show through everything else. A hand-written `importJson` may carry the directive too, making a pasted section exact instead of a merge.

The defaults are the schema's, beneath `$NANOBOT_VOICE_DEFAULTS` in the gateway environment when set: a partial section, as a JSON file path or the JSON itself, laid under `channels.voice` wherever the section is resolved. A key the section leaves unset, in either spelling and block by block, comes from it and then from the schema; what the section sets, a null included, stays. That is how a board image ships its baseline (device, audio devices, models): the panel shows it, `nanobot-voice config` prints it and `nanobot-voice sync` fetches its models. It supplies values, not activation: nanobot starts the channel only when `config.json` has a `channels.voice` section with `"enabled": true` (what the toggle writes); a missing section, or one without `enabled`, stays off, and an `enabled` in the defaults is ignored. So an image writes `{"enabled": true}` there and lets everything else come from the defaults. Reset returns to it without copying it in, so a later baseline still reaches every key the user left alone. An unreadable defaults file refuses the section, naming the variable.
...
- "agents.defaults.unifiedSession runs every channel in one session": with core's `unifiedSession` on, every channel's turns share one session, WebUI chats included, and core folds a message that arrives mid-turn into the running turn, whichever channel sent it. A message typed in the WebUI or another channel while voice is mid-turn joins the voice turn and is answered aloud, speech during another channel's turn is answered in that channel while voice waits, and a spoken stop or a barge-in cancels the other channel's turn. Leave `unifiedSession` off unless one shared conversation is worth those crossings.
