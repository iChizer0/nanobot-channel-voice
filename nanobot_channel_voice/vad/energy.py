...
# Hot path: runs per frame for the whole session; numpy-accelerated with the [ondevice] extra.
...
class EnergyVad(Vad):
    ...
    def __init__(self, fixed_threshold: float = 0.0):
        ...
        # unsynchronized, the read-modify-write pair loses the duck-synchronized floor
        # step exactly in the window it exists for.
    ...
    def is_speech(self, frame: bytes) -> bool:
        ...
        with self._lock:
            if self._noise is None:
                # Seed low so a cold start mid-speech (loud first frame) still trips
                # speech; a genuinely quiet first frame seeds itself and tracks up.
